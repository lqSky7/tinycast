...
- **Summoning animates the panel, in two halves that a single beat drives.** The panel is ordered on screen still invisible, and only then does the beat start, so the window's alpha fade and the content's own settle begin on the same frame. **The scale is the outer half and the blur is the inner one, and the split is deliberate.** `scaleEffect` sits outside `clipShape`, so the panel — scrim, hairline edge, shadow and all — is scaled whole and its silhouette stays hard; the `blur` sits *inside* `PaletteBackground`, so the beat never touches the glass, the edge stroke or the shadow. Blurring the assembled panel instead smears the hairline into a dark rim and softens the silhouette the window's shadow is computed from, which is exactly what the border did. The scale is **top-anchored, so the search bar holds its place and the list below it settles**, and it runs over the same `Duration.paletteEnter` the window fades over: one beat, two halves, so neither can finish the arrival on its own. The curve is barely front-loaded for that reason — a curve that spends its motion early hands the whole arrival to the fade, and the palette then reads as a pop. `RootPaletteView` flips one `@State` flag with an explicit `withAnimation`, the way `PanelEntrance` does, because the palette is one long-lived window with no per-summon `onAppear` to hang a beat on. Only a real summon animates: a mode switch re-shows a panel that is already up, and dropping its alpha there would blink. The whole beat is the **Summon animation** toggle in Settings › General › Appearance (`AppSettings.paletteSummonAnimation`); off, the palette is simply there, and Reduce Motion keeps the cross-fade while dropping the scale and the blur.
- **A dismissal fades only when nothing follows it.** `hide(restoreFocus:)` defers the order-out and the focus hand-back to the end of `Duration.paletteExit`, so the exit is visible; the content's exit beat is that same shorter half, so the shrink is seen while the panel is still opaque. A hide with `restoreFocus: false` is followed by an action on the app behind — a paste, a launch — so it orders out at once rather than lingering as a key window the injected keystrokes could land in.
